...
class VoiceRoles extends React.Component{
	...
	render(){
		...
		var voiceRolesChannels = channels?.filter(x => this.state.voiceRoles?.rows.some(y => y.channelId == x.id));
				...
				<Fade>
					...
					<Load loaded={this.state.voiceRoles !== null}>
						...
						<div className="inline">
							{this.state.voiceRoles?.rows.map((row, i) =>{
								return(
	...
	async componentDidMount(){
		var response = await get(`dashboard/${this.guildId}/voiceRoles`);
		...
		this.state.voiceRoles.rows = this.state.voiceRoles.rows.filter(x => this.state.voiceChannels.some(y => y.id == x.channelId))
		for(var i = 0; i < this.state.voiceRoles.rows.length; i++){
	...
	onChannelAdded(channel){
		this.settings.channels.push({ roleId: React.createRef() });
		this.state.voiceRoles.rows.push({
	...
	onChannelRemoved(id){
		...
		this.state.voiceRoles.rows = this.state.voiceRoles.rows.filter(x => x.channelId != id);
		this.setState({});
		this.props.onChanged();
	}

	getInput(){
		var rows = this.state.voiceRoles.rows;
		for(var i = 0; i < rows.length; i++){
			var card = this.settings.channels[i];
			rows[i].roleId = card.roleId.current.getValue();
		}
		this.state.voiceRoles.rows = rows;
		this.setState({});
	}
